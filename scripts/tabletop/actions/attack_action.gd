...
func _apply(unit: Unit, tabletop: Tabletop) -> int:
	...
	await unit.create_tween().tween_property(unit, 'position', dest_pos, 1).finished
	await unit.get_tree().create_timer(0.5).timeout
	unit.grid_position = start_pos
	await unit.create_tween().tween_property(unit, 'position', Vector2(start_pos * G.GRID_SIZE), 1).finished
	return base_cost
